"""Job notification services. Non-fatal by design — never re-raise."""
...
TIMEOUT_TEMPLATES_DIR = pathlib.Path(__file__).parents[2] / "templates" / "email" / "job_timeout"
...
def send_job_timeout_notification(
    job_id: uuid.UUID,
    user_id: uuid.UUID,
    user_email: str,
    display_name: str,
    ...
    """Send a job timeout email. Never raises — any failure is logged and swallowed."""
    start = time.monotonic()
    resolved_locale = "en"
    locale_source = "fallback"
    success = False
    ...
    try:
        ...
        jinja_env = jinja2.Environment(
            loader=jinja2.FileSystemLoader(str(TIMEOUT_TEMPLATES_DIR)),
            autoescape=jinja2.select_autoescape([]),
        )
        try:
            template = jinja_env.get_template(f"{resolved_locale}.txt.j2")
        ...
        jobs_url = f"{base_url.rstrip('/')}/jobs"
        title = book_title or display_name
        ...
        log_structured(
            logger,
            logging.INFO if success else logging.WARNING,
            "timeout_notification_dispatched",
            {
                "job_id": str(job_id),
                "user_id": str(user_id),
                "ui_locale": resolved_locale,
