...
- post-completion notification dispatch (non-fatal side effect after credit settlement)
...
`app/notifications/` contains the transactional email side-effect boundary.

Responsibilities:
- resolve user locale from `jobs.ui_locale`
- render the appropriate Jinja2 plain-text template
- dispatch via Resend API (integrated via `httpx`)
- emit `notification_dispatched` analytics event

Rules:
- the notification service is called **after** `session.commit()` in the worker's `_complete_job` handler — job terminal state is durable before any email is sent
- email dispatch is **non-fatal**: any failure is logged at WARNING level; the job remains `completed`
- no raw book content may appear in email payloads, subject lines, or log entries
- the notification service must not be imported by any pipeline stage module

Provider: Resend (DEC-021). New environment variables: `RESEND_API_KEY`, `RESEND_FROM_ADDRESS`, `APP_BASE_URL`.
