...
| FIX-2 | Worker startup recovery — re-enqueue orphaned created job_runs | OPS | approved | critical | small |
| FIX-3 | Skip completed translation batches on job_run resume | OPS | approved | critical | medium |
| COST-1 | Anthropic prompt caching for static system prompt | FEAT-COST-OPT | completed | high | small |
| COST-2 | Cross-chapter batch merging in batch_planner | FEAT-COST-OPT | completed | high | small |
| COST-3 | Express tier visibility in upload UI | FEAT-COST-OPT | completed | medium | medium |
| FIX-4 | Fix pre-existing test_context_string_bounded failure in ConsistencyMemory | OPS | completed | medium | small |
| DISC-1 | Discovery: optimal batch size and speed/cost optimizations for translation pipeline | FEAT-COST-OPT | completed | high | medium |
| DISC-2 | Discovery: further cost reduction — multi-model by pair/mode & alternatives | FEAT-COST-OPT | completed | high | medium |
| DISC-3 | Discovery: models by language pair (multi-vendor) + multi-provider architecture | FEAT-COST-OPT | completed | high | large |
| OPT-1 | Compact JSON output instruction in translation prompts (~15% cost saving) | FEAT-COST-OPT | completed | high | small |
| FIX-5 | Cache tokens not passed to cost ledger — overcharges after COST-1 prompt caching | FEAT-COST | planned | high | small |
| FIX-6 | Add unique constraint on (job_run_id, batch_index) in translation_batches | OPS | completed | high | small |
...
### FIX-2 — Worker startup recovery

```
task_id:              FIX-2
title:                Worker startup recovery — re-enqueue orphaned created job_runs
type:                 fix
status:               approved
priority:             critical
capability_id:        OPS
parent_feature:       OPS
source_agent:         Architect
description:          On worker startup, query PostgreSQL for job_runs with status='created'
                      whose parent job has status='queued' and re-enqueue them into Redis.
                      Prevents jobs from getting permanently stuck when Redis is cleared on
                      worker redeploy or crash.
acceptance_criteria:
  - find_unqueued_created_runs() returns only created/queued pairs
  - Leased, processing, completed, failed runs are excluded
  - Startup recovery is wrapped in try/except — cannot crash the worker
  - Each recovered run is logged as created_job_run_recovered
dependencies:
  - none
estimated_complexity: small
```
...
### FIX-3 — Skip completed translation batches on job_run resume

```
task_id:              FIX-3
title:                Skip completed translation batches on job_run resume
type:                 fix
status:               approved
priority:             critical
capability_id:        OPS
parent_feature:       OPS
source_agent:         Architect
description:          When a job_run resumes after failure, detect all-completed translation
                      batches and load the cached TranslatedSegmentCollection from S3 instead
                      of re-running all LLM calls. After successful translation, persist the
                      collection to S3 + create an Artifact DB record for future resumes.
acceptance_criteria:
  - total_batches computed from actual DB count, not planning estimates
  - All-complete + S3 hit: returns loaded collection without calling provider
  - All-complete + S3 miss: falls through to run_translation() (one-time cost for pre-fix runs)
  - S3 write + DB Artifact insert after successful run_translation()
  - Both persistence steps are non-fatal
  - translation_collection added to ARTIFACT_TYPES registry
dependencies:
  - none
estimated_complexity: medium
```
...
### TASK-78 — Add ui_locale to jobs table and job creation API

```
task_id:              TASK-78
title:                Add ui_locale to jobs table and job creation API
type:                 feature
status:               approved
priority:             high
capability_id:        FEAT-NOTIFY
parent_feature:       FEAT-NOTIFY
source_agent:         Architect
description:          Add nullable VARCHAR(5) ui_locale column to the jobs table (Alembic migration
                      0017). Update Job model, SubmitJobRequest (with max_length=5 validation), and
                      submit_job() service to persist the field. Frontend passes current Next.js
                      locale in job creation payload.
acceptance_criteria:
  - Migration 0017 applies cleanly and rolls back cleanly
  - Job.ui_locale is nullable with no default
  - POST /jobs with ui_locale stores it; without ui_locale stores null
  - SubmitJobRequest rejects values longer than 5 chars with 422
dependencies:
  - none
estimated_complexity: small
```
...
### TASK-79 — Implement Resend email client module

```
task_id:              TASK-79
title:                Implement Resend email client module
type:                 feature
status:               approved
priority:             high
capability_id:        FEAT-NOTIFY
parent_feature:       FEAT-NOTIFY
source_agent:         Architect
description:          app/notifications/email_client.py — thin httpx wrapper around Resend API.
                      send_email(to, subject, text_body) -> bool. Non-fatal by design: returns
                      False on any exception or missing RESEND_API_KEY. Never raises.
acceptance_criteria:
  - Returns False if RESEND_API_KEY not set; no HTTP call made
  - Returns True on Resend 2xx
...
### TASK-80 — Create plain-text Jinja2 email templates for 14 locales

```
task_id:              TASK-80
title:                Create plain-text Jinja2 email templates for 14 locales
type:                 feature
status:               approved
priority:             high
capability_id:        FEAT-NOTIFY
parent_feature:       FEAT-NOTIFY
source_agent:         Architect
description:          templates/email/job_completed/{locale}.txt.j2 for all 14 supported locales.
                      Each template: Subject: line, greeting with display_name, completion sentence
                      with book_title, jobs_url link, closing. Variables: display_name, book_title,
                      jobs_url.
acceptance_criteria:
  - All 14 locale files exist
  - Each renders without error given display_name, book_title, jobs_url
  - Subject: header line present in each file
  - No raw book content in templates
dependencies:
  - none
estimated_complexity: medium
```
...
### TASK-81 — Integrate notification dispatch into worker job completion handler

```
task_id:              TASK-81
title:                Integrate notification dispatch into worker job completion handler
type:                 feature
status:               approved
priority:             high
capability_id:        FEAT-NOTIFY
parent_feature:       FEAT-NOTIFY
source_agent:         Architect
description:          app/notifications/notification_service.py resolves locale from job.ui_locale,
                      renders Jinja2 template, calls email_client, emits notification_dispatched
                      analytics event. Wired into WorkerOrchestrator._complete_job after
                      session.commit(). Non-fatal: any exception logged as notification_setup_failed
                      or notification_dispatched(success=false); job remains completed.
acceptance_criteria:
  - Notification called after session.commit() in _complete_job
  - Job remains completed on any notification failure
  - notification_dispatched event emitted on success and failure paths
  - Event fields: job_id, user_id, ui_locale, locale_source, success, failure_reason, duration_ms, timestamp
  - Locale falls back to en when ui_locale is null or unsupported
  - 15 tests pass
dependencies:
  - TASK-78, TASK-79, TASK-80
estimated_complexity: small
```

---

### COST-1 — Anthropic Prompt Caching for system prompt

```
task_id:              COST-1
title:                Anthropic prompt caching for static system prompt
type:                 optimization
status:               planned
priority:             high
capability_id:        FEAT-COST-OPT
parent_feature:       FEAT-COST-OPT
source_agent:         Iteration Manager
description:          Add cache_control breakpoints to the static system prompt in
                      AnthropicProvider._call_api() so the Anthropic API can cache
                      the repeated system prompt across batches of the same job.
                      Expected savings: ~10-15% on input tokens (prompt overhead per call).
                      No behavioral change — same translation output.
acceptance_criteria:
  - cache_control breakpoint added to system prompt block
  - Anthropic API called with extended system param format (list of blocks)
  - Existing provider tests continue to pass
  - No change to translation output
dependencies:
  - none
estimated_complexity: small
```
...
```
task_id:              COST-2
title:                Cross-chapter batch merging in batch_planner
type:                 optimization
status:               planned
priority:             high
capability_id:        FEAT-COST-OPT
parent_feature:       FEAT-COST-OPT
source_agent:         Iteration Manager
description:          Modify plan_batches() in app/pipeline/segmentation/batch_planner.py
                      to merge consecutive small chapters into a single batch instead of
                      always flushing at every chapter boundary. Use TARGET_TOKENS_PER_BATCH
                      (currently unused at 2000) as the minimum fill target: only flush at
                      chapter boundary when the current batch already meets or exceeds
                      TARGET_TOKENS_PER_BATCH. Cap at MAX_TOKENS_PER_BATCH=4000.
                      Expected savings: 25-35% fewer API calls for books with many short chapters.
acceptance_criteria:
  - Adjacent chapters merged when current batch < TARGET_TOKENS_PER_BATCH
  - Flush still occurs when adding next chapter would exceed MAX_TOKENS_PER_BATCH
  - Books with chapters >= TARGET_TOKENS_PER_BATCH are unaffected (same behavior)
  - All existing batch_planner tests pass
  - New tests cover: small adjacent chapters merged, large chapters not merged
dependencies:
  - none
estimated_complexity: small
```
...
### COST-3 — Express tier visibility in upload UI

```
task_id:              COST-3
title:                Express tier visibility in upload UI
type:                 feature
status:               planned
priority:             medium
capability_id:        FEAT-COST-OPT
parent_feature:       FEAT-COST-OPT
source_agent:         Product
source_artifact:      docs/features/COST-3-express-tier-visibility.md
description:          Make the Express translation tier (Haiku model, ~5x cheaper)
                      the default selection in the upload UI and improve tier descriptions
                      so users understand the cost/quality trade-off at the point of
                      selection. Add a visible "Recommended" badge to Express. No backend
                      changes. Affects page.tsx and four existing locale message files.
acceptance_criteria:
  - Express tier is pre-selected by default when the upload form reaches the done phase
  - Express card description communicates ~5x credit advantage and suitability for most books
  - Express card carries a visible "Recommended" badge
  - Standard card description no longer contains the word "recommended"
  - All four existing locale files (en, ru, es, sr) updated with new tier descriptions
  - Switching tier selection continues to trigger credit re-estimate (no regression)
...
**Sub-tasks:**
- COST-3-T1: Change default tier to Express + update tier descriptions in i18n [MVP]
- COST-3-T2: Add "Recommended" badge to Express tier card [MVP]

---

### FIX-9 — Worker restart leaves processing runs stuck

```
task_id:              FIX-9
title:                Worker restart leaves processing runs stuck — reset to created on startup
type:                 fix
status:               completed
priority:             critical
capability_id:        OPS
parent_feature:       OPS
source_agent:         Iteration Manager
description:          When a worker is killed mid-job (Railway redeploy, crash, OOM), the
                      active job_run remains in status='processing' with an unexpired lease
                      (lease window is 8 hours). The new worker cannot acquire the lease
                      (acquire_lease only accepts status='created') and the run is not
                      in the Redis queue (it was already dequeued). The watchdog only
                      re-queues runs with expired leases — so the job is stuck for up to
                      8 hours until the lease expires.

                      Root cause: the startup recovery path (run.py lines 122-132) only
                      handles status='leased' and status='processing' runs with *expired*
                      leases via detect_expired_leases(). Runs in 'processing' with an
                      active lease that belong to a worker_id that no longer exists are
                      not recovered.

                      Fix: extend the startup recovery in run.py to also detect
                      job_runs with status='processing' (or 'leased') whose
                      lease_expires_at has NOT yet expired — i.e., orphaned by the
                      previous deployment of this worker. These should be reset to
                      'created' and re-enqueued, just as expired-lease orphans are.

                      One safe approach: on startup, mark ALL 'leased' and 'processing'
                      runs for this worker's job_ids (or globally, since only one worker
                      pool exists) as 'created' and re-enqueue — the unique constraint on
                      (job_run_id, batch_index) in translation_batches ensures
                      already-completed batches will not be duplicated (the
                      all-complete skip check in _run_translate handles the full-resume
                      case; partial-batch resumability is tracked separately in FIX-3).

                      Observed: happened three times during 2026-03-29 after redeploys
                      triggered by agent framework sync and requirements.txt fix. Each
                      time required manual ops SQL reset + Redis RPUSH.
acceptance_criteria:
  - On worker startup, any job_run with status in ('leased', 'processing') is reset
    to status='created', lease fields cleared, and re-enqueued into Redis
  - The parent job's status is set to 'queued' if it was 'processing'
  - Existing test for startup recovery (tests/worker/test_resumability.py) is extended
    to cover this scenario
  - No regression: already-completed runs (status='completed') are not touched
  - No regression: runs belonging to other in-flight workers (if horizontal scaling
