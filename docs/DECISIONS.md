...
## DEC-021 — Transactional Email Provider: Resend
...
FEAT-NOTIFY requires sending a transactional email to the user when their translation job completes. No email infrastructure existed in the project. A new external service provider must be selected and documented per ARCHITECTURE_GUARDRAILS Rules 8 and 9.

**Decision**

Use **Resend** as the transactional email provider, integrated via a thin `httpx` wrapper with no additional Python SDK dependency. Email templates are stored as Jinja2 `.txt.j2` files under `templates/email/job_completed/`.

**Rationale**

- Resend has no sandbox approval process; the free tier (3,000 emails/month) covers early-stage volume without cost.
- Integration is a single authenticated POST to the Resend REST API. `httpx` is already a declared project dependency, so no new package is added.
- The integration is a side effect at the system boundary (worker `_complete_job` post-commit). It does not touch pipeline stage logic, resumability, or credits.
- Switching providers requires changing only the HTTP call target and the API key environment variable.

**Consequences**

- New environment variables required: `RESEND_API_KEY` (secret), `RESEND_FROM_ADDRESS` (configurable, default `Unfolda <noreply@unfolda.app>`), `APP_BASE_URL` (existing or new).
- The sending domain must be DNS-verified in the Resend dashboard before production use (DKIM, SPF).
- Email failures are non-fatal and must never fail the job. The `notification_dispatched` analytics event records success and failure for operational visibility.
- If email volume exceeds the Resend free tier or if the project moves fully to AWS, a migration to AWS SES (using the existing `boto3` dependency) is the natural follow-up path.

---

## DEC-022 — Unique constraint on translation_batches (job_run_id, batch_index)
...
The orchestrator counts `translation_batches` rows per `job_run_id` to decide whether all batches are complete and whether an S3 artifact can be trusted for resume. Without a uniqueness guarantee, a failed resume could insert a second row for the same `batch_index`, doubling counts and breaking FIX-3 skip logic.

**Decision**

Enforce `UNIQUE (job_run_id, batch_index)` on `translation_batches`. Before applying the constraint, migration `0018` deletes duplicate rows, keeping one per pair: prefer `status = 'completed'`, then latest `completed_at`, then latest `created_at`.

**Consequences**

- `INSERT` of a second row for the same batch index fails with an integrity error; orchestration must not double-complete batches.
- Production upgrade runs the dedupe SQL once; rows removed are superseded duplicates only.
